...
- **Reels:** active — Veo 3.1 (`veo-3.1-generate-preview`, 720p), 4 scenes × 4s = 16s, migrated from Kling Sept 2026 (Kling account balance became unusable; see changelog below). Published resolution is unaffected — `apply-brand-video.js` upscales to 1080×1920 during branding regardless of source resolution.
...
**What changed in `generate-reel.js`:** same exported `generateReel(record, ctx)` interface, so `pipeline.js` needed no logic changes beyond comment wording. Internally: `predictLongRunning` + polling instead of Kling's submit/poll pair; `ctx.imageUrl` is now downloaded and embedded as base64 (Veo's image-to-video API takes inline bytes, not a URL Veo fetches server-side like Kling did); `aspectRatio: '9:16'`, `personGeneration: 'allow_adult'` (with an automatic retry without it if a region/account rejects that value — a real risk flagged during evaluation, though it didn't hit this account); same negative-prompt content, ported to Veo's `negativePrompt` parameter. Veo's own file-download URL requires an `x-goog-api-key` header and expires, so `generate-reel.js` downloads and re-uploads to Cloudinary itself before returning — `apply-brand-video.js` and `fetch-guard.js` still only ever see Cloudinary URLs, unchanged from the Kling era. `fetch-guard.js`'s `ALLOWED_DOMAINS` swapped `klingai.com` → `generativelanguage.googleapis.com` for the one new outbound fetch (downloading the Veo clip).

**Known limitation — forced audio:** the `veo-3.1-generate-preview` model generates native audio on every clip with no way to disable it via the raw REST API (`generateAudio: false` is rejected: `"generateAudio isn't supported by this model"`). This doesn't break anything — `apply-brand-video.js`'s FFmpeg mix already discards all source audio via `-map 0:v` and mixes in only `assets/music/`'s own track — but it means every clip is billed at Veo's audio-inclusive rate with no way to opt out on this model.

**Cost-optimized 2026-09-16** after the initial migration's cost (~A$77/month at 1080p/8s-default) was flagged as too high. Verified live against the API: Veo's per-second rate is genuinely identical across resolutions (A$0.556/sec either way — confirmed via Cloud Console billing), so resolution alone changes nothing. The real finding: **duration is resolution-gated**. 1080p only accepts `durationSeconds: 8` (its own default — 4 and 6 are both rejected: `"1080p is not supported for a duration of N seconds"`); 720p accepts 4 (odd values like 5 or 7 are rejected everywhere, at any resolution). So 720p isn't cheaper per second — it's the *only* resolution that unlocks a shorter, cheaper clip at all. `generate-reel.js` now uses `RESOLUTION = '720p'`, `DURATION_SECONDS = 4`, cutting real cost to ~A$38.55/month — about 50% down, verified end-to-end (image → Veo → Cloudinary → full branding pipeline with music) before shipping. Published resolution is unaffected (`apply-brand-video.js` upscales 720p source to 1080×1920 during branding), but total reel length dropped from 20s to 16s (`SCENE_SECS` in `apply-brand-video.js` updated from 5 to 4 to match) since the clip itself is now genuinely 4s rather than an 8s clip trimmed down. See `docs/SERVICES_AND_COSTS.md` for full cost history and the reasoning trail.

**Operational note:** the Veo/Gemini API account runs on prepaid credits, not a postpaid invoice — it hit zero balance mid-migration (a `429 "Your prepayment credits are depleted"`, structurally identical in spirit to the original Kling balance failure this migration exists to fix) and needed a manual top-up at ai.studio/projects before testing could continue. Check the balance ahead of each reel cycle until this is confirmed stable in practice.
...
- **Caption length cut 2026-09-09:** marketing flagged that post copy read as too long/dense in the review dashboard. `single_photo`/`reel` captions dropped from 150–280 words (including hashtags) to 70–130, hashtags from 10–15 to 8–12, with an explicit "cut any line that doesn't add new information" instruction (`generate-content.js`'s CAPTION FORMAT section). Carousel captions dropped from 120–180 words to 50–90 including hashtags, 8–12 hashtags, max 3–4 body lines (`generate-carousel.js`) — reasoning: the carousel slides already carry the content, so the caption's job is just to hook and push "desliza para ver más," not restate the carousel. `humanize-caption.js`'s final polish pass was updated to match (70–130 words) and now explicitly cuts an overlong draft down as part of the edit rather than only reformatting it in place.
...
- **Audience is Latin America broadly, updated 2026-08-08, redone 2026-09-09:** GlobeHop serves students across all of LatAm — copy always defaults to "estudiantes latinoamericanos" or direct address to the reader, and never singles out Colombia or any other specific country as the audience (previously Colombia was permitted "when genuinely Colombia-specific"; that exception was removed after posts kept referring to Colombia specifically). Updated in `generate-content.js`, `generate-carousel.js`, `generate-news.js`, and `humanize-caption.js`. **The Aug 8 version of this fix never actually landed in these files** — marketing flagged on 2026-09-09 that captions were still generating "colombianos" language weeks later, and an audit found `humanize-caption.js` (the final caption-polishing pass) still literally instructed "Colombian register," `generate-content.js`/`generate-carousel.js` still had the old company description and the Colombia exception clause, and all 23 profiles in `src/utils/characters.js` still said "Colombian" rather than "Latin American" despite this file claiming otherwise. Most likely cause: the Aug 13 git filter-branch used to scrub leaked secrets from history (see git-workflow notes) dropped the commit — no commit for this change exists in `git log` between the Aug 8 date and the next real commit. Redone 2026-09-09 across `humanize-caption.js`, `generate-content.js`, `generate-carousel.js`, `generate-news.js`, `src/utils/characters.js` (including removing a literal "yellow Colombia football jersey" wardrobe detail on one profile — a visual signal that would have bled into generated images), `src/utils/variety.js`, `scripts/test-kling-v2.js`, and `docs/GUIA_EQUIPO_MARKETING.md`. If this recurs, check `git log` for the relevant files first — CLAUDE.md claiming a fix landed is not proof it's still in the code.
...
Character profiles are defined in `src/utils/characters.js`. Full prompts were originally sourced from `Review/GlobeHop_Female_Male_Character_Library.md` (11 female + 12 male profiles, appearance descriptors rooted in 7 Colombian regions — Medellín, Cali, Barranquilla, Bogotá, Cartagena, Pereira, Bucaramanga). They are injected into image prompts only (not captions or hooks) via a `CHARACTER LOCK` block in the Claude user message. **Genericized from "Colombian" to "Latin American" 2026-08-08, redone 2026-09-09** (appearance, skin tone, and regional variety are unchanged — only the explicit country label was changed) to stop posts reading as Colombia-exclusive; the internal `CO_FEMALE_MEDELLIN_01`-style region comments are left as-is since they never reach the prompt text. The 2026-09-09 pass also removed a literal "yellow Colombia football jersey" wardrobe detail on one female profile (`CO_FEMALE_BOGOTA_02`) — the only place a Colombia-specific visual cue, not just a text label, could have appeared directly in a generated image.
...
- Model: `veo-3.1-generate-preview` (Gemini Developer API), `resolution: '720p'`, `durationSeconds: 4`, `aspectRatio: '9:16'`, `personGeneration: 'allow_adult'` (auto-retries without this param if an account/region rejects it) — 720p/4s is a deliberate cost optimization, not a quality default; see the migration changelog above for why
...
- `durationSeconds: 4` is rejected with a fallback to the model default if Veo's accepted duration set ever changes for this resolution — see `submitTask()`'s retry logic in `generate-reel.js`
- Published resolution is unaffected — `apply-brand-video.js` upscales the 720p source back to 1080×1920 during branding regardless
...
- **Hook structure & caption format (`pickHookStructure`, `pickCaptionFormat`) — fixed 2026-09-09:** these two pickers (8 hook rhetorical structures, 4 caption formats A-D, both injected as MANDATORY locks) originally keyed only on `weekIndex(fecha)`, with no day-slot mixed in — an ultrareview audit found this meant **every post Mon/Wed/Fri/Sat of the same calendar week resolved to the identical hook structure and identical caption format**, forcing all 4 of that week's captions into the same rhetorical opener and paragraph shape. This directly fed the "looks like one repetitive page" complaint the locks were built to prevent, and had gone unnoticed because `pickAustraliaLocation`/`selectCharacter`/`pickSceneArchetype` all correctly mix in the day slot already — these two were the only variety pickers that didn't. Fixed by adopting the same day-slot pattern (`weekIndex(fecha) * 4 + slot`); verified empirically that every week now gets 4 distinct hook structures and all 4 caption formats.
...
**Resolved 2026-08-12, actually landed 2026-09-09 — Parent/child decision-maker framing retired.** Marketing flagged a live carousel that addressed parents ("Dejaste ir a tu hija...") — all copy must speak to the prospective student directly, never their parents. Decision: retire, not rework. As with the Colombia framing fix above, this note originally claimed the retirement was complete on 2026-08-12, but an ultrareview audit on 2026-09-09 found `AUDIENCE_ROTATION` was still a 20-slot array with `padres` at 2 slots (10%), `generate-content.js`'s AUDIENCE SEGMENTS block still described `padres`, and `generate-carousel.js` still had `T03 Parent Content` wired into its template table and arc-exception list — none of the claimed removals had actually landed. Fixed for real 2026-09-09: `padres` removed from `src/utils/pillar-rotation.js`'s `AUDIENCE_ROTATION` (now 18 slots, 100% student-facing — universitarios 6, profesionales 5, estudiantes_secundaria 4, adultos 3, same relative proportions renormalized), `src/generate-content.js`'s AUDIENCE SEGMENTS block, and `src/generate-carousel.js`'s `T03 Parent Content` template row + arc-exception reference. `scripts/setup-airtable.js`'s `Audiencia` field choices and legacy seeding `AUDIENCES` array, and `scripts/demo-week.js`'s demo record, still reference `padres` — **not fixed in this pass** (out of scope for the flagged bug; the Airtable schema choice list also requires explicit confirmation before editing per the Guard Rails section, and neither script sits on the live seeding path since `reset-airtable.js`/`seed-next-weeks.js` both import `AUDIENCE_ROTATION` from `pillar-rotation.js`, not the local `AUDIENCES` array). The already-queued 2026-08-12 `padres`-audience carousel record was left untouched by request — this only prevents future posts from targeting parents.
...
- `generate-content.js`: injected as an optional "REFERENCE SOURCES" block for `destination_spotlight`, `student_story`, `agency_promo`, and `city_spotlight` pillars only (never `visa_tip` or `student_life`, which use the stricter vetted-facts block below instead; never `news_update`, which has its own dedicated NEWS LOCK) — Claude may ground one detail if it fits the TOPIC LOCK angle, must paraphrase (no URLs, no exact figures), and must never represent the Maria-from-Colombia story or YouTube testimonial video as an actual GlobeHop client
- Returns `''` if the doc is missing/unparsable — grounding is optional, never blocks generation

### Vetted facts (`getVettedFactsBlock()`, added 2026-09-09)

`visa_tip` and `student_life` previously had zero grounding — they were instructed to describe process/structure but never state any specific requirement as fact, since the team had no vetted source to cite. Marketing asked for official-source grounding (studyaustralia.gov.au specifically), so a stricter, separate mechanism was added rather than opening these two pillars up to the same paraphrase-only `REFERENCE SOURCES` block:
- A `## Vetted facts` table was added to the bottom of `docs/latam_students_australia_sources.md`, containing facts fetched directly from studyaustralia.gov.au on 2026-09-09 (student visa subclass 500, the Genuine Student requirement, OSHC, English-test proof, the official 10-step application process, the exact "48 hours in a fortnight" work-hour-cap wording, and the first-week checklist — USI, bank account, TFN, SIM, the 7-day address-notification visa condition 8533, O-Week), **expanded the same day** with `cricos.education.gov.au` (CRICOS registration check) and a visa-application-fee entry corroborated against `immi.homeaffairs.gov.au` (that domain 403s automated fetches, so the fee fact is cross-verified via independent corroboration, not a first-hand fetch — flagged as such in the doc). Each row cites its source URL. `studyinaustralia.gov.au` (with "in") was evaluated and rejected as a separate source — connection refused on every fetch attempt, and its likely content overlaps entirely with the already-verified `studyaustralia.gov.au`. `immerse.education` and `migrationdownunder.com` were suggested by the team but excluded from this table — they're commercial/private sites, not government sources, so they don't meet this table's "official only" bar.
- `parseVettedFacts()` / `getVettedFactsBlock()` in `src/utils/sources.js` parse that table the same way `parseSources()` parses the main source list — team edits the markdown table, no code change needed.
- `generate-content.js` and `generate-carousel.js` inject this as a `VETTED FACTS` / `DATOS VERIFICADOS` block for `visa_tip`/`student_life` only, permitting Claude to state **one** fact from that table as actual fact (not just paraphrase) if it fits the topic — everything else in those two pillars still can't be stated as fact, and the existing "redirect to GlobeHop, requirements change" caution stays mandatory regardless of whether a fact is cited.
- **Dollar figures are still banned everywhere**, including the vetted-facts flow — the financial-capacity fact in the table exists to name the requirement category, not the number, and the prompt instructions repeat this explicitly since it's the one fact in the table with a real figure attached at the source.
