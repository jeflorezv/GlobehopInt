// Shared singleton — both generate-content.js and generate-carousel.js import from here.
// Tracks the last MEMORY_SIZE picks so the same location is never repeated in a batch.

const MEMORY_SIZE = 7; // never repeat within the last 7 picks (~one full week of posts)
...
export function pickAustraliaLocation() {
  const available = AUSTRALIA_LOCATIONS
    .map((loc, i) => ({ loc, i }))
    .filter(({ i }) => !recentIndexes.includes(i));

  const { loc, i } = available[Math.floor(Math.random() * available.length)];

  recentIndexes.push(i);
  if (recentIndexes.length > MEMORY_SIZE) recentIndexes.shift();
